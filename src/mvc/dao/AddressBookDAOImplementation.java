    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mobileno='"+person.getMob()+"',email='"+person.getEmail()+"',address='"+person.getAddress()+" ' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setMobile(rs.getString("mobileno"));
